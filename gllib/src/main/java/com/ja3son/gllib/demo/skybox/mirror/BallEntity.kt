...
import com.ja3son.gllib.demo.skybox.mirror.Constant.ANGLE_SPAN
...
class BallEntity(val scale: Float) : BaseEntity() {
    ...
    override fun initVertexData() {
        ...
        val texCoorArray = generateTexCoor(
                (360 / ANGLE_SPAN).toInt(),
                (180 / ANGLE_SPAN).toInt()
        )
        var tc = 0
        val ts = texCoorArray.size

        val alVertix = ArrayList<Float>()
        val alTexture = ArrayList<Float>()
        ...
        while (vAngle > -90) {
            ...
            while (hAngle > 0) {
                ...
                var xozLength = scale * Constant.UNIT_SIZE * Math.cos(Math.toRadians(vAngle.toDouble()))
                ...
                val y1 = (scale * Constant.UNIT_SIZE * Math.sin(Math.toRadians(vAngle.toDouble()))).toFloat()

                xozLength = scale * Constant.UNIT_SIZE * Math.cos(Math.toRadians((vAngle - ANGLE_SPAN).toDouble()))
                ...
                val y2 = (scale * Constant.UNIT_SIZE * Math.sin(Math.toRadians((vAngle - ANGLE_SPAN).toDouble()))).toFloat()
                ...
                val y3 = (scale * Constant.UNIT_SIZE * Math.sin(Math.toRadians((vAngle - ANGLE_SPAN).toDouble()))).toFloat()

                xozLength = scale * Constant.UNIT_SIZE * Math.cos(Math.toRadians(vAngle.toDouble()))
                ...
                val y4 = (scale * Constant.UNIT_SIZE * Math.sin(Math.toRadians(vAngle.toDouble()))).toFloat()
        ...
        val texCoor = FloatArray(alTexture.size)
        for (i in 0 until alTexture.size) {
            texCoor[i] = alTexture[i]
        }
