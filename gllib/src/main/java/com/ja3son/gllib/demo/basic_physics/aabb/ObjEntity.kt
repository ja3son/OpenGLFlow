...
class ObjEntity(private val fName: String) : BaseEntity() {
    ...
    override fun initVertexData() {
        ...
        vertices_data = ShaderUtils.vXYZ!!
        ...
        vCounts = vertices_data.size / 3
        verticesBuffer = ByteBuffer.allocateDirect(vertices_data.size * FLOAT_SIZE)
                .order(ByteOrder.nativeOrder()).asFloatBuffer()
                .put(vertices_data).position(0) as FloatBuffer

        GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, vertexBufferId)
        GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, vertices_data.size * FLOAT_SIZE, verticesBuffer, GLES30.GL_STATIC_DRAW)
    ...
    override fun initShader() {
        program = ShaderUtils.createProgram(
                ShaderUtils.loadFromAssetsFile("basic_physics/obj_tex_vertex.glsl"),
                ShaderUtils.loadFromAssetsFile("basic_physics/obj_tex_fragment.glsl")
        )
    }
