...
void main() {
    ...
    ambientZM=ambientTemp;
    diffuseZM=diffuseTemp;
    specularZM=specularTemp;
    pointLight(normalize(-aNormal), ambientTemp, diffuseTemp, specularTemp, uLightLocation, vec4(0.1, 0.1, 0.1, 1.0), vec4(0.7, 0.7, 0.7, 1.0), vec4(0.3, 0.3, 0.3, 1.0));
    ambientFM=ambientTemp;
    diffuseFM=diffuseTemp;
    specularFM=specularTemp;
    u_clipDist = dot(aPosition.xyz, uParams.xyz) + uParams.w;
}